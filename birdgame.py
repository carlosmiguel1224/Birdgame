...
def receive_data(client):
    ...
    while True:
        try:
            ...
            if other_players:
                other_playes = True
            print(f"These are other_players when receiving {other_players}")
            break
        except:
            break
...
def send_data(client):
    ...
    while True:
        try:
            ...
            other_playes = False
            reconnect(client)
            print(f"These are other_players when sending {other_players}")
            if other_players:
                other_playes = True
            break
        except:
...
def main():
    ...
    while running:
        ...
        # Clear the screen
            ...
            print(f"These are important things game_ended: {game_ended} player.game_done {player.game_done} playercollide: {player.rect.colliderect(spiketop.rect)}, {player.rect.colliderect(spikebottom.rect)} ")
            game_ended = all(player[0]['game_done'] == True for player in other_players)
            #print(f"This is game ended {player.game_done}")
                # Drawing code goes here
            if player.rect.colliderect(spiketop.rect) or player.rect.colliderect(spikebottom.rect) or game_ended and player.game_done:
